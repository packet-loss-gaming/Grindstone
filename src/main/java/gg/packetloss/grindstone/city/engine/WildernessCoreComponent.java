...
public class WildernessCoreComponent extends BukkitComponent implements Listener, Runnable {
  ...
  private static CombatantExtractor<Player, LivingEntity, Projectile> extractor = new CombatantExtractor<>(
      Player.class,
      LivingEntity.class,
      Projectile.class
  );
  ...
  public void onPortalForm(PortalCreateEvent event) {
    ...
    if (event.getWorld().getName().startsWith(config.cityWorld)) {
      event.setCancelled(true);
    }
  }
  ...
  public void onPlayerInteract(PlayerInteractEvent event) {
    ...
    if (isWildernessWorld(player.getWorld())) {
      ...
      if (held == null || !ItemUtil.isPickAxe(held.getTypeId())) {
        return;
      }
      ...
      if (origin.getType().isSolid() && target.getType() == Material.AIR && WGBukkit.getPlugin().canBuild(player, target)) {
        if (!player.getInventory().removeItem(new ItemStack(BlockID.TORCH)).isEmpty()) {
          return;
        }
  ...
  public void onEntityDamage(EntityDamageEvent event) {
    ...
    if (!(entity instanceof Player) || ignoredDamage.contains(event.getCause())) {
      return;
    }
  ...
  public void onMobDeath(EntityDeathEvent event) {
    ...
    if (!(entity instanceof Monster) || event.getDroppedExp() < 1) {
      return;
    }
    ...
    if (block.getTypeId() == BlockID.LOG) {
      ...
      do {
        ...
        for (Location aLocation : nearbyLocs) {
          if (aLocation.getBlock().getTypeId() == BlockID.LEAVES) {
            return true;
          }
        }
  ...
  public void onExplosionPrime(ExplosionPrimeEvent event) {
    ...
    if (!isWildernessWorld(loc.getWorld())) {
      return;
    }
  ...
  public void onEntityExplosion(EntityExplodeEvent event) {
    if (!isWildernessWorld(event.getLocation().getWorld())) {
      return;
    }
