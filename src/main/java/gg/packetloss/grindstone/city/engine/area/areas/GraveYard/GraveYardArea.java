...
public class GraveYardArea extends AreaComponent<GraveYardConfig> {
  ...
  public void run() {
    ...
    for (LivingEntity entity : getContained(LivingEntity.class)) {
      ...
      if (entity instanceof CaveSpider && entity.getLocation().getBlock().getLightFromSky() >= 10) {
        for (int i = 0; i < 20; ++i) {
          getWorld().playEffect(entity.getLocation(), Effect.MOBSPAWNER_FLAMES, 0);
        }
        entity.remove();
        continue;
      }
  ...
  protected boolean accept(BaseBlock baseBlock, Set<BaseBlock> baseBlocks) {
    for (BaseBlock aBaseBlock : baseBlocks) {
      if (baseBlock.equalsFuzzy(aBaseBlock)) {
        return true;
      }
    }
    return false;
  ...
  private boolean isEvilMode(Block block) {
    ...
    if (EnvironmentUtil.isNightTime(getWorld().getTime()) || getWorld().hasStorm()) {
      return true;
    }
  ...
  private boolean isInEvilRegion(Location location) {
    for (ProtectedRegion region : new ProtectedRegion[] {temple}) {
      if (LocationUtil.isInRegion(getWorld(), region, location)) {
        return true;
      }
    }
  ...
  protected void localSpawn(Player player, boolean bypass) {
    if (!ChanceUtil.getChance(3) && !bypass) {
      return;
    }
  ...
  private void arm(Entity e, boolean allowItemPickup) {
    ...
    if (ChanceUtil.getChance(50)) {
      if (ChanceUtil.getChance(15)) {
        equipment.setArmorContents(ItemUtil.DIAMOND_ARMOUR);
      } else {
        equipment.setArmorContents(ItemUtil.IRON_ARMOUR);
      }
    ...
    if (ChanceUtil.getChance(50)) {
      ...
      if (ChanceUtil.getChance(35)) {
        sword = new ItemStack(ItemID.DIAMOND_SWORD);
      }
      ItemMeta meta = sword.getItemMeta();
      if (ChanceUtil.getChance(2)) {
        meta.addEnchant(Enchantment.DAMAGE_ALL, ChanceUtil.getRandom(5), false);
      }
      if (ChanceUtil.getChance(2)) {
        meta.addEnchant(Enchantment.DAMAGE_ARTHROPODS, ChanceUtil.getRandom(5), false);
      }
      if (ChanceUtil.getChance(2)) {
        meta.addEnchant(Enchantment.DAMAGE_UNDEAD, ChanceUtil.getRandom(5), false);
      }
      if (ChanceUtil.getChance(2)) {
        meta.addEnchant(Enchantment.FIRE_ASPECT, ChanceUtil.getRandom(2), false);
      }
      if (ChanceUtil.getChance(2)) {
        meta.addEnchant(Enchantment.KNOCKBACK, ChanceUtil.getRandom(2), false);
      }
      if (ChanceUtil.getChance(2)) {
        meta.addEnchant(Enchantment.LOOT_BONUS_MOBS, ChanceUtil.getRandom(3), false);
      }
  ...
  public boolean checkPressurePlateLock() {
    for (Map.Entry<Location, Boolean> lever : pressurePlateLocks.entrySet()) {
      if (!lever.getKey().getBlock().getChunk().isLoaded()) {
        return false;
      }
      Lever aLever = (Lever) lever.getKey().getBlock().getState().getData();
      if (aLever.isPowered() != lever.getValue()) {
        return false;
      }
    }
  ...
  protected void resetPressurePlateLock() {
    ...
    for (Location entry : pressurePlateLocks.keySet()) {
      if (!entry.getBlock().getChunk().isLoaded()) {
        entry.getBlock().getChunk().load();
      }
  ...
  private void breakBlock(Entity e, Location location) {
    ...
    for (BlockFace face : targets) {
      ...
      if (!BlockType.canPassThrough(bBlock.getTypeId())) {
        continue;
      }
