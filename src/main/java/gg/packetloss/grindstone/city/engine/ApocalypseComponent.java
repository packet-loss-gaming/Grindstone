...
public class ApocalypseComponent extends BukkitComponent implements Listener {
  ...
  private static CombatantExtractor<LivingEntity, LivingEntity, Projectile> extractor = new CombatantExtractor<>(
      LivingEntity.class,
      LivingEntity.class,
      Projectile.class
  );
  ...
  public void onEntityTarget(EntityTargetLivingEntityEvent event) {
    ...
    if (!(target instanceof Player) || !targeter.isValid() || !attackMob.isInstance(targeter)) {
      return;
    }
  ...
  private void cleanupRespawnPoint(Location respawnPoint) {
    ...
    if (safeRespawnRadius < 4) {
      safeRespawnRadius = 4;
    }

    for (Entity entity : respawnPoint.getWorld().getEntitiesByClass(attackMob)) {
      if (!(entity instanceof LivingEntity) || !checkEntity((LivingEntity) entity)) {
        continue;
      }
      if (entity.getLocation().distanceSquared(respawnPoint) < safeRespawnRadius) {
        entity.remove();
      }
  ...
  public void onLightningStrikeEvent(LightningStrikeEvent event) {
    ...
    if (mobCount >= mobCountMax || world.getEntities().size() > (mobCountMax * 2)) {
      return;
    }
