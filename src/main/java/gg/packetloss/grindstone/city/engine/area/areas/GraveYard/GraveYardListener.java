...
public class GraveYardListener extends AreaListener<GraveYardArea> {
  ...
  public void onPrayerApplication(PrayerApplicationEvent event) {
    if (parent.isHostileTempleArea(event.getPlayer().getLocation())) {
      event.setCancelled(true);
    }
  }
  ...
  public void onThunderChange(ThunderChangeEvent event) {
    if (!event.getWorld().equals(parent.getWorld())) {
      return;
    }
    if (event.toThunderState()) {
      ...
      for (Player player : server.getOnlinePlayers()) {
        if (player.isValid() && LocationUtil.isInRegion(parent.getWorld(), parent.rewards, player)) {
  ...
  public void onLightningStrike(LightningStrikeEvent event) {
    ...
    if (parent.getWorld().equals(world) && world.isThundering()) {
      for (Location headStone : parent.headStones) {
        if (world.getEntitiesByClass(Zombie.class).size() > 1000) {
          return;
        }
  ...
  public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
    ...
    if (parent.isHostileTempleArea(event.getEntity().getLocation())) {
      ...
      if (attacker instanceof Player) {
        ...
        if (parent.getWorld().isThundering()) {
          return;
        }
    ...
    if (entity.getCustomName() != null) {
      ...
      if (customName.equals("Grave Zombie")) {
        ...
        while (it.hasNext()) {
          ...
          if (stack != null && stack.getTypeId() == ItemID.ROTTEN_FLESH) {
            it.remove();
          }
        }
      ...
      if (entity instanceof CaveSpider) {
        ...
        while (it.hasNext()) {
          ...
          if (stack != null && !ChanceUtil.getChance(15)) {
            if (stack.getTypeId() == ItemID.STRING) {
              it.remove();
            }
            if (stack.getTypeId() == ItemID.SPIDER_EYE) {
              it.remove();
            }
          }
        }
        ...
        while (it.hasNext()) {
          ItemStack stack = it.next();
          if (stack != null && !ChanceUtil.getChance(15)) {
            if (stack.getTypeId() == ItemID.SULPHUR) {
              it.remove();
            }
          }
        }
  ...
  public void onPlayerPortal(PlayerPortalEvent event) {
    if (parent.isHostileTempleArea(event.getFrom()) && event.getCause().equals(PlayerTeleportEvent.TeleportCause.NETHER_PORTAL)) {
      ...
      if (tg == null) {
        tg = parent.getWorld().getSpawnLocation();
      }
  ...
  public void onPlayerTeleport(PlayerTeleportEvent event) {
    ...
    if (parent.isHostileTempleArea(event.getTo()) && !parent.admin.isAdmin(player)) {
      if (!watchedCauses.contains(event.getCause())) {
        return;
      }
  ...
  public void onPlayerDeath(PlayerDeathEvent event) {
    ...
    if (contained || player.getWorld().isThundering()) {
      ...
      if (!contained || parent.admin.isAdmin(player)) {
        return;
      }
