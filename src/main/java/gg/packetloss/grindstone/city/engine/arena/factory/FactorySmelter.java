...
public class FactorySmelter extends FactoryMech {
  ...
  public List<ItemStack> process() {
    ...
    if (availableLava < requestedLava) {
      if (maxIron > 0) {
        maxIron = maxIron - ironRemainder;
      }
      if (maxGold > 0) {
        maxGold = maxGold - goldRemainder;
      }
    }
    ...
    for (int i = maxIron; i > 0; --i) {
      product.add(new ItemStack(ItemID.IRON_BAR));
    }
    for (int i = maxGold; i > 0; --i) {
      product.add(new ItemStack(ItemID.GOLD_BAR));
    }
    return product;
  }
}
