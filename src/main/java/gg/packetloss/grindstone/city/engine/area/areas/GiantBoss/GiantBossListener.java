...
public class GiantBossListener extends AreaListener<GiantBossArea> {
  ...
  public void onPrayerApplication(PrayerApplicationEvent event) {
    if (parent.contains(event.getPlayer()) && event.getCause().getEffect().getType().isHoly()) {
      event.setCancelled(true);
    }
  }
  ...
  public void onCreepSpeak(CreepSpeakEvent event) {
    if (parent.contains(event.getPlayer(), 1) || parent.contains(event.getTargeter(), 1)) {
      event.setCancelled(true);
    }
  }
  ...
  public void onEntityDamageEvent(EntityDamageEvent event) {
    ...
    if (event.getCause().equals(EntityDamageEvent.DamageCause.BLOCK_EXPLOSION) && !parent.contains(defender) && parent.contains(defender, 1)) {
      event.setCancelled(true);
      return;
    } else if (!parent.contains(defender, 1)) {
      return;
    }
    ...
    if (attacker != null && !parent.contains(attacker, 1) || !parent.contains(defender, 1)) {
      return;
    }
      ...
      if (parent.boss != null && e instanceof Giant) {
        ...
        if (amt != 0) {
          for (Player aPlayer : players) {
            if (parent.admin.isAdmin(aPlayer)) {
              continue;
            }
