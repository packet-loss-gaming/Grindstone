...
import gg.packetloss.grindstone.prayer.impl.InventoryPrayer;
...
public class CursedMine extends AbstractRegionedArena implements MonitoredArena, Listener {
  ...
  private static CombatantExtractor<LivingEntity, Player, Arrow> extractor = new CombatantExtractor<>(
      LivingEntity.class,
      Player.class,
      Arrow.class
  );
  ...
  public void drain() {
    ...
    for (Entity e : getContained(InventoryHolder.class)) {
      try {
        org.bukkit.inventory.Inventory eInventory = ((InventoryHolder) e).getInventory();

        if (e instanceof Player) {

          if (adminComponent.isAdmin((Player) e)) {
            continue;
          }
  ...
  private boolean checkInventory(Player player, ItemStack[] itemStacks) {
    for (int aItem : items) {
      if (player.getInventory().containsAtLeast(new ItemStack(aItem), 1)) {
        return true;
      }
    }
    return false;
    ...
    try {
      if (ChanceUtil.getChance(player.getLocation().getBlockY())) {
        if (ChanceUtil.getChance(2)) {
          switch (ChanceUtil.getRandom(6)) {
            ...
            case 6:
              ...
              prayerComponent.influencePlayer(player,
                  PrayerComponent.constructPrayer(player, new InventoryPrayer(id, 64), 5000));
              break;
            default:
              break;
          }
          ...
          if (modifiedLoc != null) {
            player.teleport(modifiedLoc, PlayerTeleportEvent.TeleportCause.UNKNOWN);
          }
        }
      }
    } catch (UnsupportedPrayerException ex) {
