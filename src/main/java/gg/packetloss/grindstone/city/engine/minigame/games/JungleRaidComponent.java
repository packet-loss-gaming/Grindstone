...
package gg.packetloss.grindstone.city.engine.minigame.games;
...
import gg.packetloss.grindstone.city.engine.minigame.GameProgress;
import gg.packetloss.grindstone.city.engine.minigame.MinigameComponent;
...
public class JungleRaidComponent extends MinigameComponent {
  ...
  public boolean addToTeam(Player player, int teamNumber, Set<Character> flags) {
    ...
    for (int i = 0; i < 2; i++) {
      gear.add(arrows.clone());
    }

    player.getInventory().addItem(gear.toArray(new ItemStack[gear.size()]));

    ItemStack[] leatherArmour = ItemUtil.LEATHER_ARMOUR;
  ...
  public void printFlags() {
    ...
    if (gameFlags.contains('H')) {
      ChatUtil.sendNotice(players, "Hunter Mode");
    }
    if (gameFlags.contains('G')) {
      ChatUtil.sendNotice(players, "Guilds Enabled");
    }
    ...
    if (gameFlags.contains('h')) {
      ...
      for (Player player : players) {
        ...
        for (int i = 0; i < inventory.getContents().length; i++) {
          ...
          if (stack == null || stack.getTypeId() == ItemID.COOKED_BEEF) {
            continue;
          }
    ...
    if (gameFlags.contains('d')) {
      ChatUtil.sendWarning(players, "Death touch");
    }
    if (gameFlags.contains('a')) {
      ChatUtil.sendWarning(players, "2012");
    }
    if (gameFlags.contains('p')) {
      ChatUtil.sendNotice(players, "Potion Plummet");
    }
    ...
    if (gameFlags.contains('f')) {
      ChatUtil.sendNotice(players, ChatColor.AQUA, "No fire spread");
    }
    if (gameFlags.contains('m')) {
      ChatUtil.sendNotice(players, ChatColor.AQUA, "No mining");
    }
    if (gameFlags.contains('b')) {
      ChatUtil.sendNotice(players, ChatColor.AQUA, "No block break");
    }

    if (gameFlags.contains('q')) {
      ChatUtil.sendNotice(players, ChatColor.GOLD, "Quick start");
    }
    if (gameFlags.contains('S')) {
      ChatUtil.sendNotice(players, ChatColor.GOLD, "Sudden death disabled");
    }
  }
  ...
  public void removeGoneFromTeam(Player player, boolean forced) {
    ...
    if (!goneState.containsKey(player.getName())) {
      return;
    }
  ...
  public void run() {
    ...
    try {
      if (!isGameInitialised() && !queue.isEmpty()) {
        processQueue();
      }
      if (playerState.size() == 0 && !isGameInitialised()) {
        return;
      }
      ...
      for (PlayerGameState entry : playerState.values()) {
        try {
          Player player = Bukkit.getPlayerExact(entry.getOwnerName());
          if (player == null || !player.isValid() || contains(player.getLocation())) {
            continue;
          }
          player.setHealth(0);
        } catch (Exception e) {
      ...
      if (gameFlags.contains('a') || gameFlags.contains('g') || gameFlags.contains('p') || suddenD) {
        ...
        for (int i = 0; i < ChanceUtil.getRangedRandom(amt / 3, amt); i++) {
          ...
          if (testLoc.getBlock().getTypeId() != BlockID.AIR) {
            continue;
          }
        ...
        if (amt < 150 && ChanceUtil.getChance(gameFlags.contains('s') ? 9 : 25)) {
          amt++;
        }
      }
  ...
  private class JungleRaidListener implements Listener {
    ...
    public void onItemDrop(PlayerDropItemEvent event) {
      ...
      if (getTeam(event.getPlayer()) != -1 && !isGameActive()) {
        event.setCancelled(true);
      }
    }
      ...
      if (getTeam(player) != -1) {
        if (stack != null && stack.getTypeId() == ItemID.COMPASS) {
          ...
          for (PlayerGameState state : playerState.values()) {
            ...
            // Check validity
            if (aPlayer == null || !player.isValid() || player.equals(aPlayer)) {
              continue;
            }

            // Check team
            if (isFriendlyFire(player, aPlayer)) {
              continue;
            }
