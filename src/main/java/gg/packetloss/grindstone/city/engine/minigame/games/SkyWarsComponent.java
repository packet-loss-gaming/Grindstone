...
public class SkyWarsComponent extends MinigameComponent {
  ...
  private static CombatantExtractor<Player, Player, Snowball> extractor = new CombatantExtractor<>(
      Player.class,
      Player.class,
      Snowball.class
  );
  ...
  private void awardPowerup(Player player, ItemStack held) {
    ...
    if (ChanceUtil.getChance(12)) {
      ...
      if (ItemUtil.matchesFilter(held, ChatColor.AQUA + "Sky Feather [Doom]", false)) {
        return;
      }
  ...
  public void printFlags() {
    ...
    if (gameFlags.contains('r')) {
      ChatUtil.sendNotice(players, ChatColor.GOLD, "Regen enabled");
    }
    if (gameFlags.contains('c')) {
      ChatUtil.sendNotice(players, ChatColor.GOLD, "Chicken++");
    }
  }
  ...
  public Entity[] getContainedEntities(int parentsUp, Class<?>... classes) {
    ...
    for (Entity entity : world.getEntitiesByClasses(classes)) {
      ...
      if (entity.isValid() && LocationUtil.isInRegion(r, entity)) {
  ...
  public void run() {
    ...
    try {
      ...
      for (int i = 0; i < playerState.size(); i++) {
        ...
        if (!ChanceUtil.getChance(10) && !gameFlags.contains('c')) {
          continue;
        }
            ...
            case RIGHT_CLICK_AIR:
              ...
              while (it.hasNext()) {
                ...
                for (Entity aEntity : targets) {
                  innerLoop:
                  {
                    if (!aEntity.isValid() || aEntity.equals(player)) {
                      break innerLoop;
                    }
