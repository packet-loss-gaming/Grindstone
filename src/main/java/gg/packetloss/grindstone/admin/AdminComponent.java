...
public class AdminComponent extends BukkitComponent implements Listener {
  ...
  public boolean depowerPlayer(final Player player) {
    ...
    if (worldEdit().getSession(player).hasSuperPickAxe()) {
      worldEdit().getSession(player).disableSuperPickAxe();
    }
    if (godComponent.hasGodMode(player)) {
  ...
  public boolean deguildPlayer(Player player) {
    if (ninjaComponent.isNinja(player)) {
      ninjaComponent.unninjaPlayer(player);
    }
    if (rogueComponent.isRogue(player)) {
      rogueComponent.deroguePlayer(player);
    }
    return true;
  }
  ...
  public class NestedPermissionsCommands {
    ...
    public void userGroupSetCmd(CommandContext args, CommandSender sender) {
      ...
      for (String aGroup : permission.getPlayerGroups((World) null, player)) {
        if (aGroup.equalsIgnoreCase("platinum") || aGroup.equalsIgnoreCase("admin")) {
          continue;
        }
