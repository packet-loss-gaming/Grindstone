...
public class LocationUtil {
  ...
  public static Location findFreePosition(final Location pos, boolean center) {
    ...
    if (center) {
      if (mPos.getX() == mPos.getBlockX()) {
        mPos.add(.5, 0, 0);
      }
      if (mPos.getZ() == mPos.getBlockZ()) {
        mPos.add(0, 0, .5);
      }
    }
    return mPos;
  ...
  public static boolean isCloseToPlayer(Location location, int distance) {
    final int squareDistance = distance * distance;

    for (Player player : location.getWorld().getPlayers()) {
      if (player.getLocation().distanceSquared(location) <= squareDistance) {
        return true;
      }
    }
    return false;
  ...
  public static boolean getBelowID(Location loc, int id) {
    ...
    if (searchBlock.getTypeId() == id) {
      return true;
    }
    if (searchBlock.getTypeId() != BlockID.AIR) {
      return false;
    }

    for (BlockFace blockFace : SURROUNDING_BLOCK_FACES) {
      if (searchBlock.getRelative(blockFace).getTypeId() == id) {
        return true;
      }
    }
    return false;
